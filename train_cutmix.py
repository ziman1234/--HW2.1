...
parser.add_argument('--augtype', type=str, default='', help="cutout,mixup or cutmix? Default:``")
parser.add_argument('--model_path', type=str, default="", help="load model path.")
opt = parser.parse_args()
print(opt)
writer = SummaryWriter('cutmix')

try:
  os.makedirs("./cutmix_checkpoints")
except OSError:
  pass
...
def train(train_dataloader, model, criterion, optimizer, epoch):
  ...
  for i, data in enumerate(train_dataloader):
    ...
    output = model(inputs)
    if p < 0.5:
      loss = cutmix_criterion(output, targets)
    else:
  ...
  writer.add_scalar(tag="acc/prec5", scalar_value=top5.avg,
                  global_step=epoch)
  torch.save(model.state_dict(), f"./cutmix_checkpoints/{opt.datasets}_epoch_{epoch + 1}.pth")
