...
parser.add_argument('--augtype', type=str, default='mixup', help="cutout,mixup or cutmix? Default:``")
parser.add_argument('--model_path', type=str, default="", help="load model path.")
opt = parser.parse_args()
print(opt)
writer = SummaryWriter('mixup')
try:
  os.makedirs("./mixup_checkpoints")
except OSError:
  pass
...
def train(train_dataloader, model, criterion, optimizer, epoch):
  ...
  writer.add_scalar(tag="acc/prec5", scalar_value=top5.avg,
                  global_step=epoch)
  torch.save(model.state_dict(), f"./mixup_checkpoints/{opt.datasets}_epoch_{epoch + 1}.pth")
